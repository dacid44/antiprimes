...
class antiprimes:
    ...
    def num_factors(self, num):
        ...
        if len(pFac) > 0:
            ...
            val = 0
            for factor in reversed(self.primes[:self.primes.index(items[-1][0])]):
...
if __name__ == '__main__':
    num_threads = 12 # num_threads = int(input('Number of threads: '))
    a = antiprimes('primes1.txt', num_threads)
    limit = 1000000 # limit = int(input('Limit: '))
    startTime = datetime.datetime.now()
    toPrint = a.mpAPrimes(limit)
